package  {
	...
	public class shadowExam extends ResponsiveScene {
		...
		override public function onReady():void {
			stop();
			var zgarbiSound:Sound = new Sound(new URLRequest("sounds/26.mp3")); 
			zgarbiSound.play();
		...
		{
			TweenMax.killAll();
			m_scene.nextExam.removeEventListener(MouseEvent.CLICK, onNextExamClickListener);
			m_scene.prev.removeEventListener(MouseEvent.CLICK, onPrevNextClickListener);
			m_scene.next.removeEventListener(MouseEvent.CLICK, onPrevNextClickListener);
			
			for(var  i = 0; i < m_sunPlaces.length; i++ )
				m_sunPlaces[i].removeEventListener(MouseEvent.CLICK, onSunPlaceClickListener)			
		}
